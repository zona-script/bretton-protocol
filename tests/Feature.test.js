...
describe('Features', function () {
  ...
  beforeEach(async () => {
    // deploy BRET
    this.BRETToken = await ERC20Fake.new(
      'Bretton Protocol Token',
      'BRET',
      '18', // different decimal place as nToken
      { from: admin }
    )
    ...
    this.managedRewardPool = await ManagedRewardPool.new(
      '100', // 100 seconds
      this.BRETToken.address,
      { from: admin }
    )
    // mint BRET for ManagedRewardPool
    this.BRETToken.mint(this.managedRewardPool.address, '10000000000000000000000') // 10000
    // notify rewards
    await this.managedRewardPool.notifyRewardAmount('10000000000000000000000', { from: admin })

    // deploy nTokens
    this.nToken = await nToken.new(
      'nToken',
      'DTK',
      '18',
    ...
    // user approve underlying to nToken
    await this.underlyingOne.approve(this.nToken.address, '-1', { from: user }) // inifinite
    await this.underlyingTwo.approve(this.nToken.address, '-1', { from: user }) // inifinite
    // promot nToken as manager for managedRewardPool
    this.managedRewardPool.promote(this.nToken.address, { from: admin })
  })

  it('mint and redeem nToken using multiple underlyings', async () => {
    /*** MINT ***/

    // mint nToken
    await this.nToken.mint(user, this.underlyingOne.address, new BN('100000000'), { from: user }) // mint with 100 of underlying one
    await this.nToken.mint(user, this.underlyingTwo.address, new BN('100000000000000000000'), { from: user }) // mint with 100 of underlying two
    ...
    // should mint 200 nToken
    expect(await this.nToken.totalSupply.call()).to.be.bignumber.equal(new BN('200000000000000000000'))
    expect(await this.nToken.balanceOf.call(user)).to.be.bignumber.equal(new BN('200000000000000000000'))

    /*** REDEEM ***/

    // redeem nToken
    await this.nToken.redeem(user, this.underlyingOne.address, new BN('100000000'), { from: user }) // redeem 100 of underlying one
    await this.nToken.redeem(user, this.underlyingTwo.address, new BN('100000000000000000000'), { from: user })  // redeem 100 of underlying two
    ...
    // should burn 100 nToken
    expect(await this.nToken.totalSupply.call()).to.be.bignumber.equal(new BN('0'))
    expect(await this.nToken.balanceOf.call(user)).to.be.bignumber.equal(new BN('0'))
  })
  ...
  it('accure interest earnings in multiple dPools', async () => {
    ...
    // redeem
    await this.nToken.redeem(user, this.underlyingOne.address, new BN('100000000'), { from: user }) // redeem into 100 of underlying one
    ...
    await this.nToken.redeem(user, this.underlyingTwo.address, new BN('100000000000000000000'), { from: user })  // redeem into 100 of underlying two
  ...
  it('collect earnings from dPool and allow redeem', async () => {
    // mint
    await this.nToken.mint(user, this.underlyingOne.address, new BN('10000000'), { from: user }) // mint with 10 of underlying one
    await this.nToken.mint(user, this.underlyingTwo.address, new BN('10000000000000000000'), { from: user }) // mint with 10 of underlying two
    ...
    // redeem
    await this.nToken.redeem(user, this.underlyingOne.address, new BN('20000000'), { from: user }) // redeem into 20 of underlying one
    ...
    await this.nToken.redeem(user, this.underlyingTwo.address, new BN('20000000000000000000'), { from: user })  // redeem into 20 of underlying two
  ...
  it('swap between two underlyings', async () => {
    ...
    // swap
    await this.nToken.swap(user, this.underlyingOne.address, new BN('5000000'), this.underlyingTwo.address, { from: user }) // swap 5 underlyingOne to 5 underlyingTwo
  ...
  it('mine BRET token from minting nToken', async () => {
    ...
    // mint some nToken
    await this.nToken.mint(user, this.underlyingTwo.address, new BN('10000000000000000000'), { from: user }) // mint 10 nTokens
    ...
    // should have mined BRETToken for minter, who has whole share of pool

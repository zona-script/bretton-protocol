...
/**
  * @title Bretton's InterestRateModel Implementation
  * @notice Calculate the per block borrow and supply rate
  * @author Bretton
  */
contract InterestRateModel is InterestRateModelInterface {
    ...
    */
    ...
    constructor(uint baseRatePerYear, uint multiplierPerYear, uint blocksPerYear_) public {
        ...
        require(multiplierPerBlock > 0, 'multiplierPerBlock must be greater than 0');

        emit NewInterestParams(baseRatePerBlock, multiplierPerBlock, blocksPerYear);
    }
    ...
    /**
      * @notice Calculates the current borrow interest rate per block: `baseRatePerBlock + utilizationRate * multiplierPerBlock`
      * @param cash The amount of cash in the market
      * @param borrows The amount of borrows in the market
    ...
    /**
      * @notice Calculates the current supply interest rate per block: `utilizationRate * borrowRate * (1 - reserveFactor)`
      * @param cash The amount of cash in the market
      * @param borrows The amount of borrows in the market
      * @param reserves The amount of reserves in the market

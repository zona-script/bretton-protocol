...
contract Controller is Exponential, Ownable, ControllerErrorReporter, ControllerInterface {
     ...
     */
    function getAssetsIn(address account) external view returns (BTokenInterface[] memory) {
        BTokenInterface[] memory assetsIn = accountAssets[account];
     ...
     */
    function checkMembership(address account, BTokenInterface bToken) external view returns (bool) {
        return markets[address(bToken)].accountMembership[account];
    }

    /**
     * @notice Add assets to be included in account liquidity calculation
     * @param bToken The address of the bToken market to be enabled
    ...
    function redeemVerify(address bToken, address redeemer, uint redeemAmount, uint redeemTokens) external {
        ...
        // Shouldn't be able use 0 bTokens to redeem >0 underlying
    ...
    function liquidateBorrowAllowed(
        ...
        uint borrowBalance = BTokenInterface(bTokenBorrowed).borrowBalanceStored(borrower);
        (MathError mathErr, uint maxClose) = mulScalarTruncate(Exp({mantissa: markets[bTokenBorrowed].closeFactorMantissa}), borrowBalance);
    ...
    function getAccountLiquidityInternal(address account) internal view returns (Error, uint, uint) {
        return getHypotheticalAccountLiquidityInternal(account, BTokenInterface(0), 0, 0);
    }
    ...
    function getHypotheticalAccountLiquidityInternal(
        ...
        BTokenInterface[] memory assets = accountAssets[account];
        for (uint i = 0; i < assets.length; i++) {
            BTokenInterface asset = assets[i];
    ...
    /**
      ...
      */
    function _setPriceOracle(PriceOracleInterface newOracle) onlyOwner public returns (uint) {
        newOracle.isPriceOracle(); // sanity check
    ...
    /**
      ...
      */
    function _supportMarket(address bToken) onlyOwner external {
        BTokenInterface(bToken).isBToken(); // Sanity check to make sure its really a BToken

        require(!markets[bToken].isListed, "market already listed");

        // Add to market mapping
        markets[bToken] = Market({
                                            isListed: true,
        ...
        for (uint i = 0; i < allMarkets.length; i ++) {
            require(allMarkets[i] != BTokenInterface(bToken), "market already added");
        }
        allMarkets.push(BTokenInterface(bToken));
    ...
    /**
      ...
      */
    function _setMaxAssets(uint newMaxAssets) onlyOwner external {
     ...
     * @param bToken The address of the market (token) to set closeFactor for
     * @param newCloseFactorMantissa New close factor, scaled by 1e18
     * @return uint 0=success, otherwise a failure. (See ErrorReporter for details)
     */
    function _setCloseFactor(address bToken, uint newCloseFactorMantissa) onlyOwner external returns (uint) {
        ...
        uint oldCloseFactorMantissa = markets[bToken].closeFactorMantissa;
        markets[bToken].closeFactorMantissa = newCloseFactorMantissa;
        emit NewCloseFactor(bToken, oldCloseFactorMantissa, newCloseFactorMantissa);
    ...
    /**
      ...
      * @return uint 0=success, otherwise a failure. (See ErrorReporter for details)
      */
    function _setCollateralFactor(address bToken, uint newCollateralFactorMantissa) onlyOwner external returns (uint) {
        ...
        uint oldCollateralFactorMantissa = markets[bToken].collateralFactorMantissa;
        markets[bToken].collateralFactorMantissa = newCollateralFactorMantissa;
